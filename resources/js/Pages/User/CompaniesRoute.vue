...
import ViewModal from "../../Widgets/ViewModal.vue";
...
function toggle(id) {
    data.company = companies.filter((company) => {
        if (company.id === id) {
            return company
        }
    })[0]
...
<template>
    <ViewModal v-if="isModalOpen" :label="'Company'"></ViewModal>
    <section>
        <div class="grid">
            <div @click="toggle(company.id)" class="card bg-gray-50 lg:col-span-1 md:col-span-3" v-for="company in companies">
                <span class="icon">
                  <svg
                      viewBox="0 0 50 50"
                      fill="none"
