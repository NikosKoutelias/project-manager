...
function toggle(id) {
    data.project = projects.filter((project) => {
        if (project.id === id) {
            return project
        }
    })[0]
...
<template>
    <ViewModal v-if="isModalOpen" :label="'Project'"></ViewModal>
    <section>
        <div class="grid">
            <div @click="toggle(project.id)" class="card bg-gray-50 lg:col-span-1 md:col-span-3"
